...
class StockTradingEnv(gym.Env):
    ...
    def __init__(self, 
        ...
        self.data = self.df.loc[self.df.day == self.day, :]
        self.terminal = False     
    ...
    def _buy_stock(self, index, action):
        ...
        def _do_buy():
            if self.state[index+1].item()>0: 
                #Buy only if the price is > 0 (no missing data in this particular date)       
                available_amount = self.state[0] // self.state[index+1]
                # print('available_amount:{}'.format(available_amount))
                
                #update balance
                buy_num_shares = min(available_amount.item(), action.item())
    ...
    def step(self, actions):
        ...
        if self.terminal:
            ...
            df_rewards = pd.DataFrame([[x] for x in self.rewards_memory])
            ...
            log.info("environment/portfolio_value", end_total_asset)
            log.info("environment/total_reward", tot_reward)
            log.info("environment/total_reward_pct", (tot_reward / (end_total_asset - tot_reward)) * 100)
            log.info("environment/total_cost", self.cost)
            log.info("environment/total_trades", self.trades)
